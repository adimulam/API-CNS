...
def preprocess_data(data):
    ...
    data['Result'] = data['Result'].replace({'negative': 0, 'positive': 1}).astype(int)
...
def log_to_mlflow(model, X_train, X_test, y_train, y_test):
    with mlflow.start_run():
        ...
        mlflow.log_param("max_iterations", model.max_iter)
...
def main():
    # Load the dataset
    data = pd.read_csv("../data/MedicalDatasetForModel.csv")  
